...
/**
 * @Author: duanlei
 * @Date: 2019/3/11 2:24 PM
 * @Version 1.0
 * 客户端logger
 */
public class ClintLogger {
    ...
    public static void setNeedEnableLogger(boolean needEnableLogger) {
        ClintLogger.needEnableLogger = needEnableLogger;
    }
     ...
     * 通讯错误日志
     */
    public static void logCommError(Exception e, HttpURLConnection conn, String appKey, String method, Map<String, MapValue> params) {
        if (!needEnableLogger) {
            return;
        }
    ...
    public static void logCommError(Exception e, String url, String appKey, String method, Map<String, MapValue> params) {
        if (!needEnableLogger) {
            return;
        }
    ...
    private static void logCommError(Exception e, HttpURLConnection conn, String appKey, String method, String content) {
        Map<String, MapValue> params = parseParam(content);
        _logCommError(e, conn, null, appKey, method, params);
    }
     ...
     * 通讯错误日志
     */
    private static void _logCommError(Exception e, HttpURLConnection conn, String url, String appKey, String method, Map<String, MapValue> params) {
    ...
    private static Map<String, MapValue> parseParam(String contentString) {
        Map<String, MapValue> params = new HashMap<String, MapValue>();
        ...
        if (paramsArray != null) {
            for (String param : paramsArray) {
                ...
                if (keyValue != null && keyValue.length == 2) {
                    MapValue mapValue = new MapValue(ParaType.STRING, keyValue[1]);
                    params.put(keyValue[0], mapValue);
                }
            }
        }
        return params;
